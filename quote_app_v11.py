...
st.title("🚚 EPIG Quote Finder — v11 (Trailer picker only for Hotshot/Semi)")
...
ALL_VEHICLES = {"progressive","geico"}  # accept all vehicle types
...
if trailers_allowed:
    ...
    st.sidebar.caption("Trailer Type is only applicable for Hotshot or Semi-truck. Hidden for Cargo Van and Box Truck.")
...
cargo_selected = st.sidebar.multiselect("Cargo Commodities", cargo_list, default=[])
...
# MVR Age removed from sidebar; internal default will be used below
...
# Optional Details: allow override of operation_class even when trailers are hidden
with st.sidebar.expander("Optional Details"):
    op_choices = ["dry_van","flatbed","reefer","straight","intermodal","non_tanker","heavy_haul"]
...
if submitted:
    ...
    st.caption("Trailer selection now only shows for Hotshot/Semi. Cargo Van defaults to non_tanker; Box Truck defaults to straight. You can still override operation class in Optional Details.")
