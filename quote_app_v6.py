...
# Carriers that only write semis
SEMI_ONLY = {"bulldog national rrg","southwind rrg","berkley small business"}

# Carriers lenient on experience/license (no-CDL experience allowed)
...
def evaluate_carrier(carrier, rules_df, inputs):
    ...
    # Special vehicle rule: Bulldog/Southwind/Berkley => only 'semi'
    ...
    # Required keys vary by carrier (Progressive/GEICO get leniency on experience & license)
...
# Vehicle type with friendly labels
...
# Auto-populate operation_class from trailer selection (default dry_van)
operation_class = trailer_selected[0] if trailer_selected else "dry_van"

with st.sidebar.expander("Optional Details"):
    # Show operation_class but allow agent to override if necessary
...
user_inputs = {
    ...
    "operation": norm(operation_class),  # keep in sync
...
if submitted:
    ...
    if all(r["status"]=="Ineligible" for r in results):
        st.info("If everything is **Ineligible**, double-check the Risk State, vehicle type, UIIA, and excluded cargo/operations.")
    ...
    st.caption("Vehicle types updated (Cargo Van, Hotshot, Box Truck, Semi-truck). Trailer multiselect added and auto-sets operation class (default Dry Van).")
