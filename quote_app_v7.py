...
vehicle_display = st.sidebar.selectbox("Vehicle Type", list(vehicle_display_to_key.keys()))
vehicle_type = vehicle_display_to_key[vehicle_display]

trailer_choices = ["dry_van", "flatbed", "reefer", "straight", "intermodal", "non_tanker", "heavy_haul"]
trailer_selected = st.sidebar.multiselect("Trailer Type(s)", trailer_choices, default=["dry_van"])

# Cargo commodities
cargo_all = ['Air Conditioning Equipment', 'Air Freight (Mail, UPS, & FedEx)', 'Aircraft-- Engines', 'Aircraft-- Parts', 'Amazon Goods', 'Appliances-- Major', 'Appliances-- Small', 'ATVs, Snowmobiles & Waverunners', 'Audio, Video & Studio Equipment', 'Auto Parts & Accessories (not Tires)', 'Bakery Goods & Products', 'Batteries', 'Beverage-- Beer', 'Beverage-- Liquor', 'Beverage-- Non-Alcoholic', 'Beverage-- Wine', 'Bicycles, Segways & Hoverboards', 'Books', 'Bottles-- Glass', 'Bottles-- Plastic', 'Building Materials (non-flatbed)', 'Cable & Wire (not Copper/Fiber Optics)', 'Cable & Wire-- Fiber Optics', 'Camera Supplies & Accessories', 'Cameras & Photographic Equipment', 'Candy & Confectionary', 'Canned Goods', 'Carpets & Rugs (other than Oriental Rugs)', 'Cell Phones', 'Chemicals (not Red Label Placard)', 'Chemicals-- Red Label Placard', 'China, Ceramics, Glassware & Pottery', 'Cigarettes, Cigars or Tobacco (mfgd.)', 'Clothing (other than Athletic, Designer Clothing)', 'Clothing-- Athletic', 'Clothing-- Blue Jeans', 'Clothing-- Designer', 'Clothing-- T Shirts', 'Computer-- Desktop, Laptop & Network Systems', 'Computer-- Games & Software', 'Computer-- Gaming Systems', 'Containerized Freight', 'Copper and Copper Products', 'Cosmetics (not Perfume)', 'Cotton (baled cotton)', "Dept Store Merchandise (Macy's, Nordstrom's)", 'Dept Store Merchandise (Wal-Mart, Target, etc.)', 'DVD Players, MP3 Players, iPods & iPads', 'DVDs, Compact Discs & Tapes', 'Electrical Supplies (other than Copper)', 'Electronic-- Accessories', 'Electronic-- Data Processing (not Computers)', 'Explosives', 'Fashion Accessories', 'Film', 'Fine Arts', 'Firearms and Ammunition', 'Furniture (new from factory)', 'Furs', 'General Merchandise/Dry Freight', 'Glass', 'Groceries (other than produce)', 'Hardware', 'Hazardous Materials (Placards)', 'Household Goods (Movers)', 'Jewelry & Watches', 'Lawn and Garden Equipment', 'Leather and Leather Goods', 'Machinery (Light & Non-precision)', 'Medical Diagnostic Equipment', 'Medical Supplies (not Medicine or Drugs)', 'Memorabilia & Collectibles', 'Metal Products (finished-- not precious)', 'Money & Securities', 'Motorcycles', 'Office Equipment', 'Office Supplies', 'Optical Goods', 'Oriental Rugs', 'Paint', 'Paper, Paper Products & Printed Matter', 'Perfume', 'Pharmaceutical Products-- Over the Counter', 'Pharmaceutical Products-- Prescription', 'Plants-- Flowers', 'Plants-- Trees & Shrubs (other than Temp Controlled)', 'Plastic Products', 'Plumbing Supplies (other than Copper)', 'Precious Metals, Bullion & Alloys', 'Produce (other than refrigerated)', 'Purses, Handbags, Wallets & Belts', 'Radioactive Material', 'Rubber Products (other than Tires)', 'Shoes (other than Designer & Athletic)', 'Shoes-- Designer & Athletic', 'Signs', 'Silk & Silk Products', 'Soap Products, Household Cleaners', 'Sporting Goods & Equipment', 'Stereos & Radios', 'Televisions', 'Textiles (Cloth)', 'Tires', 'Tobacco (unrefined)', 'Toiletries', 'Tools', 'Toys (Hobbies & Crafts)', 'Wood Products (other than Furniture)', 'Butter', 'Cheese', 'Dairy Products', 'Eggs', 'Frozen Food (other than Meat & Seafood)', 'Ice Cream', 'Juice', 'Meat-- Boxed', 'Meat-- Hanging/Swinging', 'Milk', 'Plants-- Cut Flowers', 'Plants-- Trees & Shrubs', 'Poultry', 'Produce', 'Reefer processed foods (other than frozen)', 'Seafood-- Fresh (other than Canned Seafood)', 'Seafood-- Frozen (other than Canned Seafood)', 'Aggregate', 'Agricultural Products', 'Asphalt (Liquid)', 'Bulk Hazardous Materials (Placard)', 'Cement', 'Coal', 'Copper and Copper Products (Bulk)', 'Cotton (other than Baled Cotton)', 'Feed (Animal/Livestock)', 'Fertilizer (Nitrate)', 'Fertilizer (other than Nitrates)', 'Frac Sand', 'Fruits & Vegetables (fresh from field)', 'Garbage, Trash & Refuse', 'Gas, Fuel & Petroleum Products', 'Grain', 'Gravel, Stone & Rock', 'Hay', 'Heating Oil', 'Liquid (Non-flammable)', 'Metals (raw, scrap, coils-- other than copper)', 'Milk (Tanker)', 'Nuts - Peanuts, Almonds, Pecans, Pistachios, etc.', 'Oil (unrefined)', 'Salt', 'Sand', 'Tar', 'Top Soil, Dirt & Fill', 'Wood Chips, Mulch & Garden Ties', 'Agricultural Equipment', 'Building Materials', 'Coiled Steel', 'Contractors Equipment', 'Copper and Copper Products (Flatbed)', 'Flatbed Cable & Wire (not Copper/Fiber Optics)', 'Flatbed Cable & Wire - Fiber Optics', 'Landscaping Products', 'Logs, Timber & Pulpwood (Logging)', 'Lumber, Pallets & Wood (processed)', 'Machinery-- Heavy > 10,000 lbs (flatbed)', 'Machinery-- Light < 10,000 lbs (flatbed)', 'Metal (other than Copper)', 'Mobile Homes (no motor)', 'Oil Field - Pipe & Valves', 'Oil Field-- Heavy Equipment', 'Oversized/Overweight Loads', 'Pipe (other than Copper)', 'Printing Presses', 'Rigging (any property requiring)', 'Solar Panels', 'Stone Slab/Products (Marble, etc.)', 'Swimming Pools, Spas & Hot Tubs', 'Trailers (New for delivery)', 'Transformers', 'Turbines', 'Antique Automobiles/Vehicles', 'New Automobiles', 'New Boats', 'Recreational Vehicles', 'Reposessed Vehicles', 'Towing', 'Used Automobiles', 'Used Boats', 'Wrecker Service', 'Cattle', 'Equine (other than Racing)', 'Equine (racing)', 'Exotic Animals (Alpaca, Llama, etc.)', 'Goats', 'Poultry (Livestock)', 'Sheep', 'Swine']
cargo_selected = st.sidebar.multiselect("Cargo Commodities", cargo_all, default=[])
...
uiia_required = st.sidebar.selectbox("UIIA Agreement Required?", ["false","true"])

operation_class = trailer_selected[0] if trailer_selected else "dry_van"

with st.sidebar.expander("Optional Details"):
    operation_override = st.selectbox("Operation Class (override)", [""] + trailer_choices, index=0)
...
user_inputs = {
    ...
    "trailer_type": [norm(x) for x in trailer_selected],
...
if submitted:
    ...
    if all(r["status"]=="Ineligible" for r in results):
        st.info("If everything is **Ineligible**, double-check the Risk State, vehicle type, UIIA, trailer & cargo selections, and excluded operations.")
    ...
    st.caption("Cargo commodities list added. Trailer selection auto-sets operation class (default Dry Van). Bulldog/Southwind/Berkley = semi only; Progressive/GEICO allow no‑CDL experience.")
