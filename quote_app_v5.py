...
# Carriers that are lenient on experience/license (no-CDL experience allowed)
...
def evaluate_carrier(carrier, rules_df, inputs):
    ...
    for _,r in rows.iterrows():
        ...
        # --- Special-case UIIA: treat as exclusion only when user selects TRUE ---
        ...
        if is_exclusion:
            if passed:
                # Friendlier phrasing for generic exclusions
...
if submitted:
    ...
    if all(r["status"]=="Ineligible" for r in results):
        st.info("If everything is **Ineligible**, double-check the Risk State, vehicle type, and excluded cargo/operations.")
    ...
    st.caption("Now with a UIIA toggle. If set to TRUE, eligible carriers that exclude UIIA/intermodal risks will show as **Ineligible** with a clear message.")
