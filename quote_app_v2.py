...
st.title("🚚 EPIG Quote Finder — v2 eligibility")
...
# Hard exclusion keys (if IN / == matches => Ineligible)
EXCLUSION_KEYS = {
...
# REQUIRED rules (must pass if present for a carrier)
REQUIRED_KEYS = {
...
# state code choices from rules
state_values = sorted({s for s in sum([to_list(v) for v in rules.get("State_Code","").fillna("").tolist()],[]) if s})
state_code = st.sidebar.text_input("Risk State (2-letter)", "TX").upper() if not state_values \
             else st.sidebar.selectbox("Risk State", options=[s.upper() for s in state_values])
...
if submitted:
    ...
    results = [evaluate_carrier(c, rules, user_inputs) for c in carriers]
    ...
    for res in results:
        with st.container(border=True):
            ...
            if res["hard_fails"]:
                st.error("Ineligible triggers:"); st.write("• " + "\n• ".join(res["hard_fails"][:10]))
            if res["required_fails"]:
                st.warning("Must-fix to qualify:"); st.write("• " + "\n• ".join(res["required_fails"][:10]))
            if res["soft_fails"]:
                st.info("Considerations:"); st.write("• " + "\n• ".join(res["soft_fails"][:10]))
            if res["matches"]:
                st.success("Matched appetite:"); st.write("• " + "\n• ".join(res["matches"][:10]))
else:
    st.caption("Fill out the form and click **Find Carriers**. This version enforces required rules (state, ELD, experience, etc.) and treats exclusions as hard fails.")
