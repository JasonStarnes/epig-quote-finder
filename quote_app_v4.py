...
def evaluate_carrier(carrier, rules_df, inputs):
    ...
    if carrier_norm in SEMI_ONLY and inputs.get("vehicle_type") != "semi":
        return {
            ...
            "hard_fails": [f"vehicle_type '{inputs.get('vehicle_type')}' not eligible — {carrier} writes **semi** only"],
        }
...
if submitted:
    ...
    st.caption("Select vehicle type (Bulldog/Southwind/Berkley = **semi only**). Progressive & GEICO allow **no‑CDL experience**. Click **Find Carriers**.")
